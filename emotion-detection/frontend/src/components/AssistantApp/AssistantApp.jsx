...
const AssistantApp = () => {
  ...
  const clearTransientBotMessages = (chatId) => {
    setChats(prev => prev.map(chat => {
      if (chat.id === chatId) {
        return {
          ...chat,
          messages: chat.messages.filter(msg => !msg.isProcessing && !msg.isThinking),
        };
      }
      return chat;
    }));
  ...
  const detectEmotion = async (audioBlob) => {
    ...
    try {
      ...
      if (text) {
        ...
        clearTransientBotMessages(activeChatId);
        ...
        addThinkingMessage(activeChatId);
  ...
  const sendMessage = async (text) => {
    ...
    if (!text.includes('Detected emotion:')) {
      ...
      addThinkingMessage(chatId);
      
      const llmResponse = await getLLMResponse(text, emotionResult);
      
      clearTransientBotMessages(chatId);
  ...
  return (
    <div className="app">
      <Sidebar 
        ...
        isBlocked={isWaitingForLLM || isProcessing}
      />
      <div className="chat-container">
        ...
        <InputArea 
          ...
          isInputBlocked={isWaitingForLLM || isProcessing}
        />
      </div>
    </div>
  );
